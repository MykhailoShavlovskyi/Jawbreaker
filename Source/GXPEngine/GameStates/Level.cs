...
namespace GXPEngine
{
	public class Level  : GameObject
	{
		...
		private List <Sprite> _tutorialList = new List<Sprite> ();
		...
		void Update ()
		{
			...
			if (_progressData.showTutorial && _whichLevel == 1) 
			{
				ShowTutorial ();
			} else 
			{
		...
		void ShowTutorial () {
			...
			{
			case 1:
				box = new Sprite ("ccup.png");  //Candy Cannon
				AddChild (box);
				box.SetXY (40, 200);
				_tutorialList.Add (box);
				break;

			case 2:
				foreach (Sprite boxs in _tutorialList) 
				{
					boxs.Destroy ();
				}
				_tutorialList.Clear ();
				box = new Sprite ("pbup.png");  //power bar
				box.SetXY (28, 70);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 3:
				...
				_tutorialList.Clear ();
				box = new Sprite ("sup.png");  //stars
				box.SetXY (_game.width/2-70, 32);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 4:
				...
				_tutorialList.Clear ();
				box = new Sprite ("ysup.png"); //more stars
				box.SetXY (400,290);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 6:
				...
				_tutorialList.Clear ();
				box = new Sprite ("lsup.png");  //level Select
				box.SetXY (_game.width-275, -2);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 7:
				...
				_tutorialList.Clear ();
				box = new Sprite ("BasicPlatform.png"); //Normal Platform
				box.SetXY (50,190);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 8:
				...
				_tutorialList.Clear ();
				box = new Sprite ("slipary.png");//slipery platform
				box.SetXY (340,320);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 9:
				...
				_tutorialList.Clear ();
				box = new Sprite ("trampoline.png");  //Bounce platform
				box.SetXY (550, 200);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 10:
				...
				_tutorialList.Clear ();
				box = new Sprite ("spikes.png");  //Slipery platform
				box.SetXY (50, 520);
				AddChild (box);
				_tutorialList.Add (box);
				break;
			...
			case 11:
				...
				_progressData.showTutorial = false;
				break;
			}
		}
		#endregion
		...
		#region *Check Hud Buttons*
		//==========================CHECK HUD BUTTONS========================
		public void CheckHudButtons()
		{
			if (Input.GetMouseButtonDown (0)
...
//			}
			...
			if (A > 0 && -B >= A)
			{
				...
				if (!(impactpoint < 0 || impactpoint > linelength))
				{
					...
					if (bounciness == 1.4f
						&& bouncySoundTimer <=0 
						&& (lastBouncyObjectx != _line.start.x ||  lastBouncyObjecty != _line.start.y ))
					{
					...
					if (!enoughAnimations
						&&  bounciness == 0.7f
						&& normalSoundTimer <=0 
						&& (lastNormalObjectx != _line.start.x ||  lastNormalObjecty != _line.start.y )) 
					{
		...
		public void CheckSphereCollision(Ball sphere, float bounciness)
		{
			if(new Vec2().Set(sphere.position.Clone().Sub(  _ball.position)).Length() < _ball.radius + sphere.radius)
			{
				...
				if (bounciness == 0.1f 
					&& stickySoundTimer <=0
					&& (lastStickyObjectx != sphere.x ||  lastStickyObjecty != sphere.y )) 
				{
				...
				if (!enoughAnimations 
					&& bounciness == 0.7f 
					&& normalSoundTimer <=0 
					&& (lastNormalObjectx != sphere.x ||  lastNormalObjecty != sphere.y )) 
				{
					enoughAnimations = true;
		...
		public void LaunchCasinoIndicator()
		{
			...
			for (int i = 1; i < 12; i++)
			{
				...
				trajectoryCircle.alpha = 1 - (float)i/17;
				trajectoryCircle.scale = 1 - (float)i/12;
		...
		public void CheckStarCollision()
		{
			foreach (Sprite other in _ball.GetCollisions())
			{
				if (other is Star)
				{
					...
					Sound click = new Sound ("starpickup.wav");
					click.Play ();
